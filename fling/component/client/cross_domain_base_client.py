...
@CLIENT_REGISTRY.register('cross_domain_base_client')
class CrossDomainBaseClient(CrossDomainClientTemplate):
    """
    Overview:
    This class is the base implementation of client in Cross-Domain Federated Learning.
    ...
    def __init__(self, args: dict, domain: str, client_id: int, train_dataset: Dataset, test_dataset: Dataset = None):
        ...
        Arguments:
            ...
            - client_id: unique id for this client.(with domain id)
        Returns:
            - None
        """
        super(CrossDomainBaseClient, self).__init__(args, domain, client_id, train_dataset, test_dataset)
    ...
    def finetune_step(self, batch_data, criterion, monitor, optimizer):
        ...
        monitor.append(
            {
                f'{self.domain}_{self.client_id}_train_acc': torch.mean((y_pred == batch_y).float()).item(),
                f'{self.domain}_{self.client_id}_train_loss': loss.item()
            },
            weight=batch_y.shape[0]
        )
    ...
    def preprocess_data(self, imgs, labels):
        return {'x': imgs.to(self.device), 'y': labels.to(self.device)}
    ...
    def train(self, lr, device=None, train_args=None):
        ...
        for epoch in range(self.args.learn.local_eps):
            for _, (imgs, labels) in enumerate(self.train_dataloader):
                preprocessed_data = self.preprocess_data(imgs, labels)
    ...
    def test(self):
        ...
        # Main test loop.
        with torch.no_grad():
            for _, (imgs, labels) in enumerate(self.test_dataloader):
                preprocessed_data = self.preprocess_data(imgs, labels)
                # Update total sample number.
