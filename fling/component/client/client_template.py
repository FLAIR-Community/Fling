...
class CrossDomainClientTemplate:
    r"""
    Overview:
        Template of client in Federated Learning.
    """
    ...
    def __init__(self, args: dict, domain: str, client_id: int, train_dataset: Dataset, test_dataset: Dataset = None):
        ...
        Arguments:
            ...
            - train_dataset: private dataset for training
            - test_dataset: private dataset for testing (Optional)
            - domain: domain name to which the client belongs.
            - client_id: unique client id for this client in domain.
        Returns:
            - None
        """
        # Model construction.
        ...
        self.model = get_model(args)
        self.device = args.learn.device
        # Specify a unique client id with domain
        self.domain = domain
        self.client_id = client_id
        # This attribute will not be set until ``self.set_fed_keys(self, keys)`` is called.
        # Only weights in ``self.fed_keys`` will be collaboratively trained using Federated Learning.
