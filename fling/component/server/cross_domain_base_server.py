...
@SERVER_REGISTRY.register('cross_domain_base_server')
class CrossDomainBaseServer(ServerTemplate):

    def __init__(self, args: Dict, test_dataset: Dataset):
        super(CrossDomainBaseServer, self).__init__(args, test_dataset)
        # get domains from args
        self.domains = args.data.domains.split(',')
        # get test dataloader[domain]
        self.test_loader = {}
