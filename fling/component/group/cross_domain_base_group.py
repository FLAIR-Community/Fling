...
@GROUP_REGISTRY.register('cross_domain_base_group')
class CrossDomainParameterServerGroup:
    r"""
    Overview:
    ...
    def __init__(self, args: dict, logger: Logger):
        ...
        Returns:
            - None
        """
        self.domains = args.data.domains.split(',') # domains
        self.first_domain = self.domains[0] # get first domain for initialize
        self.num_user = args.client.client_num
        self.clients = {domain:[] for domain in self.domains} # clients must be dict --> group.clients[domain][client_num]
        self.server = None
        self.args = args
        self.logger = logger
    ...
    def initialize(self) -> None:
        ...
        fed_keys = get_weights(
            self.clients[self.first_domain][0].model, self.args.group.aggregation_parameters, return_dict=True, include_non_param=True
        ).keys()

        # Step 2.
        self.logger.logging(f'Weights for federated training: {fed_keys}')
        glob_dict = {k: self.clients[self.first_domain][0].model.state_dict()[k] for k in fed_keys}
        ...
        self.logger.logging(str(self.clients[self.first_domain][0].model))
        self.logger.logging('All clients initialized.')
        self.logger.logging(
            'Parameter number in each model: {:.2f}M'.format(get_params_number(self.clients[self.first_domain][0].model) / 1e6)
        )

    def append(self, domain: str, client: CrossDomainClientTemplate) -> None:
        r"""
        Overview:
    ...
    def aggregate(self, train_round: int, aggr_parameter_args: dict = None) -> int:
        ...
        if aggr_parameter_args is not None:
            fed_keys_bak = self.clients[self.first_domain][0].fed_keys
            new_fed_keys = get_weights(self.clients[self.first_domain][0].model, aggr_parameter_args,
        ...
        if self.args.group.aggregation_method == 'avg':
            trans_cost = cross_domain_fed_avg(self.clients, self.args, self.server)
            self.sync()
        else:
